...
def main(args):
    ...
    # Load the specified param file that points to the main param file
    with open(args.arg_file, 'rb') as f:
        indexdict = pickle.load(f)

    indexlist = list(indexdict.values())[0]
    total_tasks = len(indexlist)

    f = Indexed_Pickle(list(indexdict.keys())[0])
    f.init_read()
