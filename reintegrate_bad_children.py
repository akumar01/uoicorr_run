...
if __name__ == '__main__':
    ...
    for bad_children_file in bad_children_files:
   	...
   	for bad_child in bad_children:
   		...
   		child_path = source_dir + '/' + exp_type + '/' + bad_child['path']
   		child_data = h5py_wrapper.load(child_path)

   		# Load master file
   		child_dir = bad_child['path'].split('/child')[0]
   		master_path = source_dir + '/' + exp_type + '/' + child_dir + '.dat'
   		master_data = h5py_wapper.load(master_path)
   		pdb.set_trace()
   		master_data = insert_data(master_data, child_data, child['idx'])
   		pdb.set_trace()
   		#h5py_wapper.save(master_path, master_data, write_mide)